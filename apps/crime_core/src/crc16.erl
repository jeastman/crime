...
%% Utility function to generate the above table.

%%make_table() ->
%%    Poly = 16#a001, % 0x8005 reflected
%%    lists:reverse(byte_loop(0, Poly, [])).

%%bin_loop(_, 8, _, Crc) ->
%%    Crc;
%%bin_loop(Byte, Bin, Poly, Crc) when ((Byte bxor Crc) band 1) == 1 ->
%%    bin_loop(Byte bsr 1, Bin + 1, Poly, (Crc bsr 1) bxor Poly);
%%bin_loop(Byte, Bin, Poly, Crc) ->
%%    bin_loop(Byte bsr 1, Bin + 1, Poly, Crc bsr 1).   

%%byte_loop(256, _, Table) ->
%%    Table;
%%byte_loop(Byte, Poly, Table) ->
%%    byte_loop(Byte + 1, Poly, [bin_loop(Byte, 0, Poly, 0) | Table]).
